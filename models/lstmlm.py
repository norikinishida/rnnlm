...
class LSTMLM(chainer.Chain):
    ...
    def __init__(self, vocab, word_dim, state_dim, BOS, EOS):
        ...
        links = {}
        # Word embedding
        links["embed"] = L.EmbedID(self.vocab_size, word_dim, ignore_label=-1)
        # LSTM
        links["W_upd"] = L.Linear(self.word_dim, 4 * self.state_dim)
        links["U_upd"] = L.Linear(self.state_dim, 4 * self.state_dim, nobias=True)
        # Output
        links["W_out"] = L.Linear(self.state_dim, self.vocab_size)
        super(LSTMLM, self).__init__(**links)
    ...
    def forward(self, seq_batch_word):
        ...
        # Initialization
        state, bos = self.make_initial_variables(batch_size=batch_size) # (batch_size, state_dim), (batch_size, 1)
        ...
        for batch_inputword in seq_batch_inputword:
            state = self.update_state(batch_inputword, state) # {str: (batch_size, state_dim)}
            batch_logits = self.predict_next_word(state["hidden"]) # (batch_size, vocab_size)
    ...
    def generate_sentence(self, initial_words):
        ...
        # Initialization
        state, bos = self.make_initial_variables(batch_size=1) # {str: (1, state_dim)}, (1, 1)
        ...
        for step in range(len(initial_words)):
            ...
            state = self.update_state(inputword, state) # {str: (1, state_dim)}
        ...
        for _ in range(50):
            # Predict the next word
            state = self.update_state(inputword, state) # {str: (1, state_dim)}
            logits = self.predict_next_word(state["hidden"]) # (1, vocab_size)
    ...
    def make_initial_variables(self, batch_size):
        """
        :type batch_size: int
        :rtype: {str: Variable(shape=(batch_size, state_dim), dtype=np.float32)}, Variable(shape=(batch_size,1), dtype=np.int32)
        """
        state = {}
        state["hidden"] = Variable(cuda.cupy.zeros((batch_size, self.state_dim), dtype=np.float32)) # (batch_size, state_dim)
        state["cell"] = Variable(cuda.cupy.zeros((batch_size, self.state_dim), dtype=np.float32)) # (batch_size, state_dim)
        bos = Variable(cuda.cupy.full((batch_size,1), self.BOS_ID, dtype=np.int32)) # (batch_size, 1)
        return state, bos

    def update_state(self, inputwords, state):
        """
        :type inputwords: Variable(shape=(batch_size,), dtype=np.int32)
        :type state: {str: Variable(shape=(batch_size, state_dim), dtype=np.float32)}
        :rtype: Variable(shape=(batch_size, state_dim), dtype=np.float32)
        """
        word_vectors = self.embed(inputwords) # (batch_size, word_dim)
        h_in = self.W_upd(word_vectors) + self.U_upd(state["hidden"]) # (batch_size, 4 * state_dim)
        cell_state, hidden_state = F.lstm(state["cell"], h_in) # (batch_size, state_dim), (batch_size, state_dim)
